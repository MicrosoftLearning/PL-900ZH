...
2.  新建画布应用程序
    ...
    -   输入“[[你的姓氏]校园安全]”作为名称，然后单击 **“保存”**。如果应用意外关闭，这将确保更改不会丢失。
        
    -   单击左上角的后退箭头（在 Power Apps 下方）返回到该应用。
...
3.  连接到数据源（访问）
    ...
    -   选择 **“访问”**，并等待“访问”实体显示在 **“你的应用数据”** 部分。
    
4.  要保留正在进行的工作，请单击 **“文件”**，然后单击 **“保存”**。使用后退箭头返回到应用。
...
1.  添加搜索框
    ...
    -   选择 **“HintText”** 属性，并输入 `"Enter visitor code"` 作为值（包括英文双引号）
...
2. 添加表单视图
   ...
   -   使用大小句柄，将窗体放置在搜索文本框下方
   
   -   选择 **“DataSource”** 属性并输入 **“访问”**
   ...
   -   在“高级”选项卡，选择 **“项目”** 属性，然后输入 `LookUp(Visits, Code = textCode.Text)` 

3. 要保留正在进行的工作，请单击 **“文件”**，然后单击 **“保存”**。使用后退箭头返回到应用。
...
4. 测试应用
   ...
   -   找到一个没有“实际开始时间”值或“实际结束时间”值的访问记录。选择并复制该访问的 **“代码”**。
...
2. 添加签入和签出按钮
   ...
   * 在属性窗格中，将按钮 **“Text”** 属性更改为 `"Check In"`（你可以在现有引号中输入）
   
   * 单击树视图中按钮名称 (Button1) 旁边的 **“...”**，选择 **“重命名”**，将名称更改为 `CheckInButton`
   
   * 单击 **“按钮”** 来插入另一个按钮
   
   * 在属性窗格中，将按钮 **“Text”** 属性更改为 `"Check Out"`（你可以在现有引号内输入）
...
3. 根据访问数据启用和禁用按钮。 
   ...
   * 选择“签入”按钮，然后在“属性”选项卡中单击按钮的 **“DisplayMode”** 属性
...
4. 如果已找到访问记录（非空）、记录状态为活动且访问已经开始（即实际开始值为非空），那么我们会启用 **“签出”** 按钮。
   ...
   * 选择“签出”按钮，然后在“属性”选项卡中单击按钮的 **“DisplayMode”** 属性
...
5. 要保留正在进行的工作，请单击 **“文件”**，然后单击 **“保存”**。使用后退箭头返回到应用。

6. 按 **F5**，运行应用。两个按钮均应禁用。输入你先前复制的代码值，然后按 **Tab** 键，将焦点从文本框移开。 **“签入”** 按钮应变为启动状态。清除搜索框的内容。
...
2. 将“高级”选项卡上的 **“OnSelect”** 属性设置为以下表达式。
   ...
   * `Patch(Visits, Visit, {'Actual Start': Now()});`.*Patch* 方法更新 **Visits** 实体，它是由 **Visit** 变量（当前访问）标识的记录。该表达式将 *“实际开始”* 字段的值设置为当前日期和时间 (*Now()* 方法）。
   * `Refresh([@Visits]);`.此表达式在基础值发生更改时刷新访问记录
   * `Set(Visit, LookUp(Visits, Code = textCode.Text));` 此表达式使用来自 Common Data Service 的新数据更新 *Visit* 变量。
   
   当用户单击此按钮时，“实际访问开始时间”将设置为当前日期和时间，并且数据将刷新。
...
4. 将“高级”选项卡上的 **“OnSelect”** 属性设置为以下表达式。
   ...
   当用户单击此按钮时，“实际结束时间”将设置为当前日期和时间，“访问的状态”记录将设置为“非活动”，并且数据将刷新。
...
![画布运行安全性应用](media/3-canvas-running.png)
